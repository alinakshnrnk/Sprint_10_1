...
def _reach_waiting_screen(main_page):
    ...
    return waiting_page, selected_price_text


def _reach_completed_screen(main_page):
    waiting_page, selected_price_text = _reach_waiting_screen(main_page)
    completed_page = waiting_page.wait_for_completed_order(timeout=60)
    return completed_page, selected_price_text
...
class TestTaxiOrderFullFlow:
    ...
    def test_submitting_order_shows_waiting_screen(self, main_page):
        waiting_page, _ = _reach_waiting_screen(main_page)
    ...
    def test_completed_order_shows_driver_info(self, main_page):
        completed_page, _ = _reach_completed_screen(main_page)
    ...
    def test_completed_order_details_price_matches_selected_tariff(self, main_page):
        completed_page, selected_price_text = _reach_completed_screen(main_page)
